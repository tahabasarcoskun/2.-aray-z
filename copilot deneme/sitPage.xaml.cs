...
<head>
  <meta charset=""UTF-8"" />
  <title>3D Roket Modeli - Telemetri G�r�nt�leyici</title>
  <style>
    body, html { 
      margin: 0; 
      padding: 0; 
...
<body>
  ...
  <div class=""controls-panel"">
    <div class=""control-title"">?? Roket Y�nelimi</div>
  ...
  <div class=""model-info"">
    ...
    <div>Model: G�zel Roketim (GLB)</div>
    <div>Durum: <span id=""model-status"">Y�kleniyor...</span></div>
  </div>
...
=======
    ...
    loader.load(
      'https://assets.local/3DModels/as�l/g�zelroketim.glb',
      function (gltf) {
        console.log('GLB model ba�ar�yla y�klendi!', gltf);
        ...
        document.getElementById('model-status').textContent = 'Y�klendi ?';
      ...
      function (progress) {
        ...
        document.getElementById('loading').textContent = `3D Model Y�kleniyor... ${percent}%`;
        console.log('Y�kleme ilerlemesi:', percent + '%');
      },
      function (error) {
        console.error('GLB model y�kleme hatas�:', error);
        document.getElementById('loading').textContent = 'Model y�kleme hatas�!';
...
>>>>>>> Stashed changes
    ...
    window.updateModelRotation = (yaw, pitch, roll) => {
      if (rocketModel) {
        ...
        console.log(`Roket y�nelimi g�ncellendi: Yaw=${yaw.toFixed(1)}�, Pitch=${pitch.toFixed(1)}�, Roll=${roll.toFixed(1)}�`);
      }
    };
    ...
    console.log('3D G�r�nt�leyici ba�lat�ld� - GLB model y�kleniyor...');
  </script>
</body>
</html>";
